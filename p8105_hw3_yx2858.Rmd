...
```{r}
instacart %>%
  filter(aisle %in% c('baking ingredients', 'dog food care', 'packaged vegetables fruits')) %>% #filters the dataset into only these three values
  group_by(aisle) %>% #groups the filtered dataset
...
```{r}
instacart %>%
  filter(product_name %in% c('Pink Lady Apples', 'Coffee Ice Cream')) %>% #filters the dataset into which "product_name" is either "Pink Lady Apples" or "Coffee Ice Cream"
  group_by(product_name, order_dow) %>% #groups the filtered dataset for each combination of product and day of the week
  ...
  pivot_wider(names_from = order_dow, values_from = Mean) #pivots the summarized dataset into each day of the week as a separate column with the mean order hour; therefore, this table becomes a 2x7 table (2 product_name, and 7 days of the week)
```
